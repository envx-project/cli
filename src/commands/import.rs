use anyhow::bail;

use super::*;
use crate::utils::config::Config;
use crate::utils::key::Key;
use crate::utils::prompt::{is_interactive, prompt_text};
use crate::utils::rpgp::get_vault_location;
use crate::utils::vecu8::ToHex;
use clap::Subcommand;
use std::fs;
use std::io::Cursor;

use pgp::{
    composed::{ArmorOptions, Deserializable},
    types::KeyDetails,
};

/// Import ascii armored keys from a file
#[derive(Parser, Debug)]
pub struct Args {
    #[command(subcommand)]
    command: Commands,
}

#[derive(Subcommand, Debug)]
pub enum Commands {
    /// Import a public key
    Pubkey { path: String },
}

pub async fn command(args: Args, _config: &mut Config) -> Result<()> {
    let mut vault_path = get_vault_location()?;
    match args.command {
        Commands::Pubkey { path } => {
            let buf = Cursor::new(
                fs::read_to_string(path).context("Failed to read file")?,
            );
            let (pubkey, _) =
                pgp::composed::SignedPublicKey::from_armor_single(buf)
                    .context("Failed to parse armored key")?;

            let fingerprint =
                pubkey.fingerprint().as_bytes().to_hex().to_uppercase();

            println!("Importing key: {}", fingerprint);

            let first_user_id = pubkey
                .details
                .users
                .first()
                .context("Failed to get user id from key")?
                .id
                .id()
                .to_hex();

            // TODO: fix this
            let primary_user_id = if only_hex(&first_user_id)
                && first_user_id.len() == 128
            {
                if !is_interactive() {
                    bail!(
                        "This key has no user id and stdin is not a terminal.\n\
                         Re-run interactively to supply a name/email for the key owner.",
                    );
                }
                println!("This key has no user id because it was generated by env-cli.");
                println!(
                    "Please enter the name and email of the owner of this key."
                );
                println!("User Id: {}", first_user_id);

                let name =
                    prompt_text("What is the name of the owner of this key?")?;
                let email =
                    prompt_text("What is the email of the owner of this key?")?;
                format!("{} <{}>", name, email)
            } else {
                first_user_id
            };

            let key = Key {
                fingerprint,
                note: "".to_string(),
                pubkey_only: Some(true),
                primary_user_id,
                uuid: None,
            };

            vault_path.push(format!("{}/public.key", &key.fingerprint));
            fs::write(
                vault_path,
                pubkey.to_armored_string(ArmorOptions::default())?,
            )?;
        }
    }

    Ok(())
}

fn only_hex(s: &str) -> bool {
    s.chars().all(|c| c.is_ascii_hexdigit())
}
