...
get_tmpfile() {
  ...
  suffix="$1"
  if has mktemp; then
    printf "%s%s.%s.%s" "$(mktemp)" "-envx" "${RANDOM_FOR_SH}" "${suffix}"
  else
    # No really good options here--let's pick a default + hope
    printf "/tmp/envx.%s" "${suffix}"
  fi
}
...
download() {
  ...
  printf "\n" >&2
  info "This is likely due to envx not yet supporting your configuration."
  info "If you would like to see a build for your configuration,"
  info "please create an issue requesting a build for ${MAGENTA}${TARGET}${NO_COLOR}:"
  info "${BOLD}${UNDERLINE}https://github.com/env-store/rusty-cli/issues/new/${NO_COLOR}"
  return $rc
}
...
unpack() {
  ...
  info "bug report at https://github.com/env-store/rusty-cli/issues"
  return 1
}
...
is_build_available() {
  ...
  if [ "${good}" != "1" ]; then
    error "${arch} builds for ${platform} are not yet available for envx"
    printf "\n" >&2
    info "If you would like to see a build for your configuration,"
    info "please create an issue requesting a build for ${MAGENTA}${target}${NO_COLOR}:"
    info "${BOLD}${UNDERLINE}https://github.com/env-store/rusty-cli/issues/new/${NO_COLOR}"
...
# defaults
if [ -z "${ENVX_VERSION-}" ]; then
  ENVX_VERSION="$DEFAULT_VERSION"
fi

if [ -z "${ENVX_PLATFORM-}" ]; then
...
if [ "$UNINSTALL" = 1 ]; then
  confirm "Are you sure you want to uninstall envx?"
  ...
  if test_writeable "$(dirname "$(which envx)")"; then
    sudo=""
    msg="Removing envx, please wait…"
  else
    warn "Escalated permissions are required to install to ${BIN_DIR}"
    elevate_priv
    sudo="sudo"
    msg="Removing envx as root, please wait…"
  fi
...
print_configuration () {
  if [ -n "${VERBOSE}" ]; then
    ...
    debug "${BOLD}Version${NO_COLOR}:       ${GREEN}${ENVX_VERSION}${NO_COLOR}"
    printf '\n'
  fi
}
...
printf "$MAGENTA"
  ...
  \].`[/  I
  /l\/j\  (]    .  O
 /. ~~ ,\/I          .               envx is now installed
 \\L__j^\/I       o               Run `envx help` for commands
  \/--v}  I     o   .
  |    |  I   _________                    
  |    |  I c(`       ')o            
  |    l  I   \.     ,/
